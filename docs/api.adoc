...
|Content path. The `type` argument is supported starting from version image:images/v-530.svg[Since version,opts=inline]. In the site-less mode, the result will be returned in the raw value, in other words, `env.source._path`.
