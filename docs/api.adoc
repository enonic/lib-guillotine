...
Aggregations allows to extract statistical data from search results. Only one of aggregation field must be not null.
...
|order: String
|How to order the results, type and direction. Supports _term (Alphabetic ordering of bucket keys) and _count (Numeric ordering of number of document in buckets) types. Default to _term ASC.
...
|interval: String
|The time-unit interval for creating bucket. Supported time-unit notations `y`, `M`, `W`,`d`,`h`,`m`,`s`.
...
|minDocCount: Int
|Only include bucket in result if number of hits equal or more than `minDocCount`.
...
|unit: String
|The measurement unit to use for the ranges. Legal values are either the full name or the abbreviation of the following: km (kilometers), m (meters), cm (centimeters), mm (millimeters), mi (miles), yd (yards), ft (feet) or nmi (nauticalmiles).
...
Only one values field must be not null.
